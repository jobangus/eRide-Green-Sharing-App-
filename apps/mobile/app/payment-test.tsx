...
export default function PaymentTestScreen() {
  ...
  const initialRideId = useMemo(() => {
    if (typeof params.rideId === 'string' && params.rideId.trim()) {
      return params.rideId.trim();
    }
    return SEEDED_RIDE_ID;
  }, [params.rideId]);
  ...
  const checkStatus = async () => {
    if (!rideId.trim()) {
      Alert.alert('Error', 'Enter a ride ID first');
      return;
    }
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      if (intent.dev_mode) {
        ...
        setPaymentStatus({
          status: 'captured',
          amount_final: intent.amount_aud,
          currency: 'aud',
        });
        return;
      }
  ...
  const resultConfig: Record<
    PaymentResult,
    { color: string; label: string; icon: React.ReactNode } | null
  > = {
    idle: null,
    success: {
      color: '#2E7D32',
    ...
    failed: {
      ...
      icon: <XCircle size={20} color="#B00020" />,
    },
    cancelled: {
      color: '#6B7280',
    ...
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        ...
        <Text style={styles.subtitle}>
          Test the payment flow using any ride ID that the logged-in user is authorised to access.
        </Text>

        <View style={styles.userBox}>
          <Text style={styles.userText}>
            Logged in as: <Text style={styles.bold}>{user?.email ?? 'Unknown user'}</Text>
          </Text>
        </View>
        ...
        <View style={styles.section}>
          ...
          <TouchableOpacity onPress={() => setRideId(SEEDED_RIDE_ID)} activeOpacity={0.8}>
            <Text style={styles.hint}>← Use seeded demo ride ID</Text>
          </TouchableOpacity>
        </View>
        ...
        {paymentStatus && (
          <View style={styles.statusCard}>
            ...
            {paymentStatus.amount_estimated != null && (
              <StatusRow
                label="Estimated"
                value={`A$${Number(paymentStatus.amount_estimated).toFixed(2)}`}
              />
            )}
            {paymentStatus.amount_final != null && (
              <StatusRow
                label="Final"
                value={`A$${Number(paymentStatus.amount_final).toFixed(2)}`}
              />
            )}
...
function TestCard({ number, result }: { number: string; result: string }) {
  return (
    <View style={styles.testCardRow}>
      ...
      <Text
        style={[
          styles.testCardResult,
          result === 'Success' ? styles.successText : styles.failText,
        ]}
      >
        {result}
      </Text>
